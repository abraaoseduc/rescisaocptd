function gerarCalculo476(data, valorPVR, tipoLancamento = 'INCLUSÃO') {
    const matricula = data.matricula || '';
    const nome = data.nome || '';
    const textoLancamento = tipoLancamento.toUpperCase();

    const textoCopia = `${matricula} ${nome} ${textoLancamento} DE DIFERENÇA DE PVR NO VALOR DE R$ ${valorPVR.toFixed(2).replace('.', ',')} REFERENTE A RESCISÃO DE CONTRATO EM ${data.dtalt}`;

    return `
        <div class="calc-card-item">
            <h4>RUBRICA 476 - DIFERENÇA DE PVR</h4>
            <div class="calc-details">
                • <strong>Valor Base PVR:</strong> R$ ${valorPVR.toFixed(2).replace('.', ',')}<br>
